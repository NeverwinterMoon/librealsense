...
Java_com_intel_realsense_librealsense_Updatable_nUpdateFirmwareUnsigned(JNIEnv *env,
    ...
    rs2_update_firmware_unsigned_cpp(reinterpret_cast<const rs2_device *>(handle), image, length,
                                     new rs2::update_progress_callback<decltype(cb)>(cb),
                                     update_mode, &e);
    handle_error(env, e);
...
Java_com_intel_realsense_librealsense_Updatable_nCreateFlashBackup(JNIEnv *env, jobject instance,
    ...
    std::shared_ptr<const rs2_raw_data_buffer> raw_data_buffer(
            rs2_create_flash_backup_cpp(reinterpret_cast<rs2_device *>(handle),
...
Java_com_intel_realsense_librealsense_Updatable_nCheckFirmwareCompatibility(JNIEnv *env,
                                                                            jobject instance,
                                                                            jlong handle,
                                                                            jbyteArray image) {
    rs2_error *e = NULL;
    auto length = env->GetArrayLength(image);
    int rv = rs2_check_firmware_compatibility(reinterpret_cast<const rs2_device *>(handle), image,
                                              length, &e);
    handle_error(env, e);
    return rv > 0;
}
...
extern "C" JNIEXPORT void JNICALL
Java_com_intel_realsense_librealsense_Device_nWriteCustomCalibrationTable(
  JNIEnv *env, jclass type,
  jlong handle,
  jbyteArray calibrationData
) {
    jbyte *calibrationDataNative = env->GetByteArrayElements(calibrationData, 0);
    jsize calibrationDataSize = env->GetArrayLength(calibrationData);

    rs2_error *error = nullptr;
    // Set calibration table
    rs2_set_calibration_table(reinterpret_cast<const rs2_device *>(handle),
                              (const void *) calibrationDataNative, calibrationDataSize, &error);
    handle_error(env, error);

    // Write calibration table
    rs2_write_calibration(reinterpret_cast<const rs2_device *>(handle), &error);
    handle_error(env, error);

    env->ReleaseByteArrayElements(calibrationData, calibrationDataNative, 0);
}
